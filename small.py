...
class RNN(nn.Module):
    ...
    def forward(self, x, lens):
        ...
        # print("rnn", out.shape)  # rnn torch.Size([64, 50, 512])
        hidden = self.fc(out[:, -1, :])
        return self.softmax(hidden), self.log_softmax(hidden)


class RNN_hanwen(nn.Module):
    def __init__(self, x_dim, e_dim, h_dim, o_dim):
        print("hanwen,single LSTM")
        super(RNN_hanwen, self).__init__()
        self.h_dim = h_dim
        self.dropout = nn.Dropout(0.2)
        self.emb = nn.Embedding(x_dim, e_dim, padding_idx=0)
        # 还可以添加两层 LSTM
        self.lstm = nn.LSTM(e_dim, h_dim, bidirectional=False, batch_first=True)
    ...
    def forward(self, x, lens):
        embed = self.dropout(self.emb(x))
        out, _ = self.lstm(embed)
        # print("hanwen", out.shape)  # hanwen torch.Size([64, 50, 256])
        hidden = self.fc(out[:, -1, :])
        return self.softmax(hidden), self.log_softmax(hidden)


class RNN_hanwen_2layer(nn.Module):
    def __init__(self, x_dim, e_dim, h_dim, o_dim):
        print("hanwen,2 LSTM layers")
        super(RNN_hanwen_2layer, self).__init__()
        self.h_dim = h_dim
        self.dropout = nn.Dropout(0.2)
        self.emb = nn.Embedding(x_dim, e_dim, padding_idx=0)
        # 还可以添加两层 LSTM，来了！
        self.lstm = nn.LSTM(e_dim, h_dim, num_layers=2, bidirectional=False, batch_first=True)
        self.fc = nn.Linear(h_dim, o_dim)
        self.softmax = nn.Softmax(dim=1)
        self.log_softmax = nn.LogSoftmax(dim=1)
...
class Model(object):
    def __init__(self, v_size):
        ...
        self.b_size = 64
        self.lr = 0.001
        # self.model = RNN_hanwen(v_size, 256, 256, 2)
        # self.model = RNN_hanwen_2layer(v_size, 256, 256, 2)
    ...
    def train(self, x_tr, y_tr, l_tr, x_te, y_te, l_te, epochs=15):
        ...
        for epoch in range(epochs):
            ...
            print("Epoch: ", epoch, "Loss: ", np.mean(losses), "ACC: ", np.mean(accu), "maxAcc", maxAcc,
                  "time: ", predict_time, "size: ", len(x_te), "average", predict_time / len(x_te))
...
if __name__ == '__main__':
    ...
    # 瀚文的单层 hanwen
    # clf.save("data/cache/hanwen_small_model_{}".format(name))
    # print("data/cache/hanwen_small_model_{}".format(name))
    # # 两层的模型 RNN——2layer
    # clf.save("data/cache/2layer_small_model_{}".format(name))
    # print("data/cache/2layer_small_model_{}".format(name))
    # 正常的模型 - 双向RNN
    clf.save("data/cache/small_model_{}".format(name))
    print("data/cache/small_model_{}".format(name))
