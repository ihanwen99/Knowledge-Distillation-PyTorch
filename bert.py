...
class BertClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, input_mask, label_ids):
        ...
        # print(input_ids.shape)  # torch.Size([16, 128]) [batch_size,max_seq]
        pooled_output = self.bert(input_ids, attention_mask=input_mask).pooler_output
        # bert output torch.Size([16, 768]) [batch_size,max_seq*6]
        # print("bert output",pooled_output.shape)
        pooled_output = self.dropout(pooled_output)  # drop out torch.Size([16, 768])
        # print("drop out",pooled_output.shape)

        # logits = self.classifier(pooled_output)  # classifier torch.Size([16, 2])

        logits = self.classifier2(self.classifier1(pooled_output))
        # print("classifier",logits.shape)
        if label_ids is not None:
            loss_fct = CrossEntropyLoss()
            # 执行的时候从这里返回
            # print("aaaaaa",logits.view(-1, self.num_labels).shape) # [16,2]
            # print("bbbbbb",label_ids.view(-1).shape,label_ids.view(-1)) # [16]
            # print("cccccc",loss_fct(logits.view(-1, self.num_labels), label_ids.view(-1)).shape,loss_fct(logits.view(-1, self.num_labels), label_ids.view(-1)))
...
def main(bert_model='bert-base-chinese', cache_dir=None,
         max_seq=128, batch_size=16, num_epochs=12, lr=2e-5, name="pda"):
    print(name)
    # def main(bert_model='bert-base-chinese', cache_dir=None,
    #          max_seq=128, batch_size=16, num_epochs=30, lr=5e-6):
    processor = Processor()
    train_examples = processor.get_train_examples('data/{}'.format(name))
    label_list = processor.get_labels()
    tokenizer = BertTokenizer.from_pretrained(bert_model, do_lower_case=True)  # 分词
    ...
    print('eval...')
    # eval_examples = processor.get_dev_examples('data/{}'.format(name))
    eval_examples = processor.get_test_examples('data/{}'.format(name))
    ...
    preTime = time.time() - time0
    print("preTime", preTime, len(preds), preTime / len(preds))
    print(compute_metrics(preds, eval_label_ids.numpy()))
    torch.save(model, 'data/cache/model_{}'.format(name))
    print('data/cache/model_{}'.format(name))
